    ...
    let arcs = 
      rm_calls
        (List.fold_left
          (fun arcs f -> List.fold_left
            (fun arcs a ->
	            ...
	            let a' = { arc_id = !nb_arcs; arc_src = f.func_exit;
                         arc_dst = a.arc_dst; arc_inst = CFG_skip "function return" } in
              f.func_exit.node_out <- a' :: f.func_exit.node_out;
              a.arc_dst.node_in <- a' :: a.arc_dst.node_in;
              incr nb_arcs;
              let a'' = { arc_id = !nb_arcs; arc_src = a.arc_src;
                          arc_dst = f.func_entry; arc_inst = CFG_skip "function entry" }
              in 
          ...
          cfg.cfg_funcs) in
    List.iter
      (fun n -> n.node_out <- rm_calls n.node_out; n.node_in <- rm_calls n.node_in)
      cfg.cfg_nodes; 
    let vars = List.fold_left
      (fun vars a ->
        match a.arc_inst with
        | CFG_assign(v,_) -> if substring v.var_name "__ret_" then v::vars else vars
        | _ -> vars
      )
      cfg.cfg_vars
      cfg.cfg_arcs
